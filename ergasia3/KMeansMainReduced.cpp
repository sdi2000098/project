...
std::map<std::string, int> readArgumentsFromFile(const std::string& filename) {
    ...
    defaults["number_of_vector_hash_tables"] = 10;
    defaults["number_of_vector_hash_functions"] = 30;
...
int main (int argc, char * argv[]){
    ...
    std::string inputFile , queryFile,configFile,inputFileReduced;
    ...
    KMeans * MyCluster = new KMeans(K,Method,KLSH,L,Kcube,M,probes,outputfileName,flag,inputFile);
    free(Method);
    delete MyCluster;
    DeleteTrain();
}